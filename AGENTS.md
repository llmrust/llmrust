...
5. `src/types.rs` — the shared type system (ChatRequest, ChatResponse, StreamChunk, Tool, etc.)
...
- `FinishReason` variants are cross-provider. New variants must have a clear, documented meaning.
...
- When adding a new provider:
  ...
  8. Add tests covering chat, stream, tool calls (if supported), and error handling
...
- **Never log**: API keys, prompt content, response text, request bodies, tool arguments, image data, full URLs.
- When location information is useful, use metadata fields like `message_count`, `tool_count`, `data_len`, `url_len`, `error_kind`.
