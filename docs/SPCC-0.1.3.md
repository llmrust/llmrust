...
> **状态**：`ACTIVE SSOT — M2 DONE（10/10 封板，public API freeze 生效）；M3 DONE（5/5 封板）；M4 DONE（4/4 封板）；M5 ACTIVE（3/4）`  
> **目标版本**：`llmrust 0.1.3`  
...
| Phase 3–5 | llmrust Owner（用户） | Notion AI | CodeBuddy | `ACTIVE` — M4 4/4 封板；M5 ACTIVE（3/4）；下一任务 `REL-003`（打标发布，待 Owner 授权） |
...
| `0.1.3 / M5 Release` | 审计并发布干净 0.1.3 | 3/4 | 75% | `ACTIVE` | `REL-003`（待 Owner 打标授权） | crates.io/docs.rs/GitHub tag 三方一致 |
...
| `REL-003` | M5 | `BLOCKED` | `REL-001`,`REL-002` + Owner 打标授权 | — | — | — | — |
