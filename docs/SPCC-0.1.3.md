...
> **⛔ 已封板 · 只读档案（SPEC-010 入库，2026-09-11）**：本规格自即日起转为**只读历史档案**，不再接受任何修改（含勘误）。
> **后继规格**：[`docs/SPCC-0.1.4.md`](SPCC-0.1.4.md) —— 0.1.4 的唯一事实源（SSOT）。
> 0.1.3 期内的遗留勘误随 0.1.4 规格 §0.5 继承登记；**本文件正文保持封板时原样，一字不改**。
...
| Phase 3–5 | llmrust Owner（用户） | Notion AI | CodeBuddy | `DONE` — M4 4/4 封板；M5 DONE（5/5 封板）；下一任务 `—（0.1.3 全部 Milestone 封板）` |
...
| `0.1.3 / M5 Release` | 审计并发布干净 0.1.3 | 5/5 | 100% | `DONE — 封板` | —（0.1.3 全部 Milestone 封板） | crates.io/docs.rs/GitHub tag 三方一致 |
...
| `PRX-002` | M3 | `DONE` | `PRX-001` | [#154](https://github.com/llmrust/llmrust/issues/154) | [#155](https://github.com/llmrust/llmrust/pull/155) | `f8ebf7582f1b7dadc02ebf1d873c3bf594b12c00` | STATE-PRX-002（本 PR） |
...
| `REL-003` | M5 | `DONE` | `REL-001`,`REL-002`,`REL-003A` | [#194](https://github.com/llmrust/llmrust/issues/194) | —（打标发布卡，无实现 PR） | 发布 commit `a4f77ef8` · tag `v0.1.3`(f4d022bd) · crates.io 2026-08-03T09:31:06Z | STATE-REL-003（本 PR） |
| `REV-001` | M5（治理复查，不计入 M5 任务数） | `DONE` | `REL-003`（0.1.3 发布后全量复查） | [#190](https://github.com/llmrust/llmrust/issues/190) | —（复查卡，无实现 PR） | —（无实现 PR；结论见 §11.1.4） | STATE-REV-001（本 PR） |
...
| `RC-001` | [#184](https://github.com/llmrust/llmrust/issues/184) | [PR #185](https://github.com/llmrust/llmrust/pull/185)（0.1.3 发布候选独立审计：docs/release/RC-0.1.3-AUDIT.md +240 唯一新增；A 四方对账（28/28 任务 Issue CLOSED+completed、零 MERGED_PENDING_STATE、七 milestone 逐 issue 实证枚举、分支清单）/ B 门禁 9 项重跑全绿（gitleaks 以 CI 在案替代）/ C 热点基线 7 文件全一致（3327/1884/1455/1524/1483/1132/1080）+ deny.toml 豁免核验 / D 发布完整性（package 77 项零禁止、版本元数据四方一致、semver 基线 SHA-256 钉死、release.yml tag-only+dry-run 就绪）/ E E2E 证据复核 + secrets 3/6 可接受性 + 每周 cron 生效 / F 日志隐私零命中 + agent_docs_validation 17 passed / G findings 11 项全 P2/P3 → 结论 GO） | 设计小样 APPROVE + 2 MUST（#184 comment 5161753085）→ 实现首轮 CHANGES 2 MUST + 1 SHOULD（#185 comment 5161965759：A-4 缺 M1 行、M2 双向失实、C-2 措辞）→ 执行侧逐 issue 实证修正并如实上报裁定事实差异（M2 挂载 6 含 #122 修复卡 / 缺口 5——架构师侧错误第四起记档、执行侧纪律正面先例第十一起）全数处置 → 复核 APPROVE + AUTHORIZE MERGE（#185 comment 5162034784）；CI run `30782032350`/`30782032356` 七项全绿（head `5758fb4`）；本地/远端/PR head 三点一致；FND-011 处置（裁定方案一）：5 张 issue 补挂 M2 milestone（#124/#127/#130/#133/#136），M2 closed=11（回证 #185 comment 5162054973，STR-002G 双卡 #122+#124 说明在案）；**架构师确认 GO 建议**（零 P0/P1、零 MERGED_PENDING_STATE、三方一致含 FND-011 已闭合、门禁全绿、发布完整性通过、E2E 证据完整）；REL-002 下发须 Owner 明确授权（§11.8） | `787d2ed0af5540b6b3c2bf84a8ab90d0eba09edc`（2026-08-03） | STATE-RC-001（本 PR） | M5 2/4（50%） | `DONE` — Notion AI（架构师） |
| `REL-002` | [#187](https://github.com/llmrust/llmrust/issues/187) | [PR #188](https://github.com/llmrust/llmrust/pull/188)（0.1.3 版本与发布元数据定稿：CHANGELOG [Unreleased]→[0.1.3] - 2026-08-03（条目零遗漏零新增——Added 5/Changed 15/Fixed 3 经 diff + 全量点数双证 + Security 段不复述事故细节与凭证）+ COMPATIBILITY Release date: 2026-08-03 + docs/release/RELEASE_CHECKLIST.md 新增（REL-003 核对表 + 不手工补发回滚口径 + hash 指引语防自引用）+ agent_docs_validation changelog_has_013_release_header 断言（17→18，可证伪红→绿）；版本四方一致（Cargo.toml/capabilities/CHANGELOG/COMPATIBILITY 均 0.1.3 + 日期）；capabilities verified_at 保持 2026-08-02；cargo check --locked 通过；package 79 项（基线 78 零漂移 + 新增 1 明示）；**预期 crate hash `0B6C3BE663D2C505BF7EE9B8E5147E851D6031F993112E17DB201CFAED5A9931`**（提交后干净工作树两包比对 identical=True——REL-003 对账基准；编辑前实证值 3B884C2D… 为方法证据） | 设计小样 APPROVE + 1 MUST + 2 待裁定项裁定（#187 comment 5162267671：T-4/T-5 与 hash 自引用修正、verified_at 保持、断言批准新增）→ T-4 package 计数熔断（#187 comment 5162375664：79 项实证 vs 裁定前提 78——架构师侧错误第五起记档、执行侧纪律正面先例第十二起）→ T-4 修正裁定 79 项（#187 comment 5162394481）→ 实现首轮 CHANGES 1 MUST（#188 comment 5162472243：PR Summary Changed 14 与实现 15 不一致——§10.1 PR Summary 失实阻塞项，零成本 body 勘误）全数处置 → 复核 APPROVE + AUTHORIZE MERGE（#188 comment 5162519088）；CI run `30785560503`/`30785560512` 七项全绿（head `83622ef`）；本地/远端/PR head 三点一致；**主干自本状态 PR 合入起成为唯一 release candidate（冻结——此后任何代码/元数据改动必须重新走任务卡）**；REL-003 打标发布须 Owner 明确授权 | `7e190375e929700b630621ccc0810c5a8f0cc3ab`（2026-08-03） | STATE-REL-002（本 PR） | M5 3/4（75%） | `DONE` — Notion AI（架构师） |
| `REL-003A` | [#191](https://github.com/llmrust/llmrust/issues/191) | [PR #192](https://github.com/llmrust/llmrust/pull/192)（发布通道启用：release.yml 新增 publish job（既有 validate/dry-run/guards-fmt-clippy 三 job 逐字节零改动）——needs [dry-run, guards-fmt-clippy] 双闸门 / environment: release / permissions id-token:write+contents:read / rust-lang/crates-io-auth-action@c6f97d42… # v1.0.5（§9.1 pin SHA，架构师实拉官方 tag 对象逐字一致）/ CARGO_REGISTRY_TOKEN env 注入（无 --token）/ crates.io 可见性有界轮询 30×10s；RELEASE_CHECKLIST 发布机制段更新（Trusted Publishing 身份 + TPO 启用时序定为 REL-003 发布成功验证后）；CHANGELOG [0.1.3] Added 一条；本卡只建通道——无 tag、无 publish、无 GitHub Release） | 发令前实证拦下结构性前提缺口（release.yml 为 REL-001 刻意 dry-run 无上传步骤 vs REL-003 DoD 要求上传）→ 前驱卡签发（#191）→ Owner crates.io Trusted Publishing 配置完成（页面实证 Owner ID 289775751 · release.yml · release 环境）→ 设计小样 APPROVE + 2 MUST + 2 待裁定项裁定（#191 comment 5162965422：needs 链补 guards-fmt-clippy、permissions 补 contents:read、TPO 启用定为发布成功后、action 必须 pin SHA）→ 实现首轮 APPROVE + AUTHORIZE MERGE（#192 comment 5163034087，架构师实拉官方 v1.0.5 tag 实证 pin SHA 逐字一致）；CI run `30790000273`/`30790000303` 七项全绿（head `33f73b3`）；本地/远端/PR head 三点一致。**流程偏差记档**：实际合并标题/body 与合并令指定内容不符（执行侧未逐字执行 --subject/--body-file）——内容与格式仍合规，不补救，记入 M5 复盘；新通道尚未触发（首次真实上传在 REL-003 tag 时） | `80214d6673ea2b7e83974c3e3503657b0e2d1695`（2026-08-03） | STATE-REL-003A（本 PR） | M5 4/5（80%） | `DONE` — Notion AI（架构师） |
| `REL-003` | [#194](https://github.com/llmrust/llmrust/issues/194) | 无实现 PR（打标发布卡，SPCC 设计如此）；过程修复三卡：[PR #195](https://github.com/llmrust/llmrust/pull/195)（gitleaks 下载健壮性：curl `-f --retry 5 --retry-all-errors --retry-connrefused --retry-delay 2` + `gzip -t` 预检 + mismatch 写 `$GITHUB_STEP_SUMMARY`，`EXPECTED_SHA256` 零改动）、[PR #196](https://github.com/llmrust/llmrust/pull/196)（`.gitleaks.toml` allowlist 由整目录 `paths` 改精确 `regexes` + gitleaks 扫描前移至 cargo 构建之前，release.yml/security.yml 同步）、[PR #197](https://github.com/llmrust/llmrust/pull/197)（gitleaks 产物落 `RUNNER_TEMP`，避免污染 clean-tree 门禁） | 发布前三证（main HEAD / 工作树干净 / 版本四方一致 0.1.3）；REL-003 基准 crate hash `765DA2D5AADB6AE789AB4FF4D9D1B22094CE96577E1828CC1C3AA0EB7557B909`（覆盖 REL-003A 入包变更，未复用 REL-002 值）；**Incident 链四次 tag 重建全程零上传**（① checksum mismatch → 架构师 20+ 第三方仓库交叉核验判定 pin 值权威、根因为下载健壮性，禁改 `EXPECTED_SHA256`；② examples 占位符误报 → 根因双重：`target/package/**` 构建副本被扫 + allowlist `paths` 在 `--no-git` 下失效（自 CI-002 起从未真正豁免，既有缺陷经本次暴露）；③ clean-tree 污染），每次重建均逐字回证旧/新 tag SHA；终局 run `30801472635` 四闸 validate/dry-run（`Packaged 79 files, 1.2MiB`）/guards-fmt-clippy/publish 全过，`Uploaded llmrust v0.1.3 to registry crates-io`（09:31:06Z，OIDC JWT→token→upload→revoke 全程正常）；三方验证：crates.io versions 含 0.1.3 且 `yanked=False`、docs.rs `0.1.3` 构建成功（架构师独立复核 ✅）、tag `f4d022bd`→`a4f77ef8`（架构师 `get_tag` 独立复核 ✅）；收尾：GitHub Release `v0.1.3`（09:54:49Z，含 FND-R2 双语 known limitation + FND-001/FND-002 如实披露，架构师独立复核 ✅）、crates.io Trusted Publishing Only 启用（执行侧 API 实证 `trustpub_only=True` + Release asset `rel003-tpo-enabled.png`；**架构师无 crates.io 直读通道，如实记为未独立核验项**）；非阻塞项：workflow 整体标 failure 仅因可见性轮询 300s < 索引传播延迟，列 0.1.4 改进（轮询提至 60×10s） | 发布 commit `a4f77ef8a3a1fc01e39ec689ca9d54c8577d9987`（2026-08-03，无实现 PR merge SHA） | STATE-REL-003（本 PR） | M5 5/5（100%）— **封板** | `DONE` — Notion AI（架构师） |
| `REV-001` | [#190](https://github.com/llmrust/llmrust/issues/190) | 无实现 PR（0.1.3 发布后全量复查卡，SPCC 设计如此） | 双边独立复查：执行侧 9 项（`FND-R1..R9`，comment 5162793973，基线 main @ `969d20e`）+ 架构师 7 项（`F-A1`/`F-A2`/`F-B1`/`F-B2`/`F-C1`/`F-D2`/`F-E1`）；执行侧对架构师 7 项**逐条表态全部「确认」并附逐字代码证据，零静默跳过**；去重后**统一发现表 15 项唯一**（交集 1 项 `F-C1` ≡ `FND-R1`，双边独立命中）；**双边一致：零 P0、零 P1**。处置分层：0.1.4 首批四卡（`FND-R2`→#198 已开 / `FND-R3` / `F-C1` / `F-B1`）、0.1.4 第二批（`F-B2`/`F-D2`/`F-E1`/`F-A2` 静默吞错族合卡 + `F-A1` 文档刷新）、技术债 `FND-R4..R8`、记录不修 `FND-R9`（涉 public API 面扩张，与 freeze 一并评估）。**架构师侧记档三项**：① `F-B1` 裁定漂移（comment 5162818427「0.1.4+ 候选」→ 本结论显式拉平为「0.1.4 修复」，前裁定作废）；② `FND-R2` 为架构师侧遗漏、执行侧独立发现的唯一 Contract 实质偏离（`CONTRACTS.md` L33 第 7 条），已随 0.1.3 Release 双语披露；③ **抽检局限**——架构师仅独立复核 `FND-R9`（`src/prelude.rs` blob `d780bf9d`，属实），`FND-R1..R8` 行号未逐一复算（大文件无法按行切片、私有仓库 raw 与 code-search 不可达），**采信不等于核验，不得表述为已全量复核** | —（复查卡，无 merge SHA）；结论签发 2026-08-03（comment 5167618048） | STATE-REV-001（本 PR） | M5 5/5（100%）— 治理复查不计数 | `DONE` — Notion AI（架构师） |
...
| 2026-08-02 | 明确 §16 阶段放行与治理裁定属架构师职责（护栏口径：不否决即通过） | 复盘批准、阶段放行、分支清理由架构师裁定执行；仅方向/范围/发布时间/成本/风险实质变化呈 Owner（须附架构师建议） | Owner |
...
### M2 相位复盘（2026-08-02，架构师裁定通过并放行 M3）

- **计划/完成**：10/10（STR-001、STR-002A、STR-002G、REA-001、REA-002、REA-003、REA-004G、REA-004O、STR-003、CAP-001），全部三方一致关账，无 MERGED_PENDING_STATE 残留。
- **一次通过率与返工**：实现 PR 一次通过 7/10；返工 3 单——STR-001（设计矛盾裁需修改 + 2 MUST）、STR-002A（热点台账归属 1 MUST）、REA-004G（4 MUST 窄口径返修）。返工主因为文档真实性与治理归属，非代码质量。
- **越界**：范围/依赖/安全零越界；流程偏差三记入档并设防呆——STR-001（预算 +37% 未先报审；合并标题绕过预检 → 标题第二关核验 + `--body-file`）、STR-002A（执行者自改热点基线 → 基线调整=架构师专属）、REA-004G（执行侧代行改 SPCC/台账并循环自称 → 追认不设先例，SPCC/台账改动一律架构师回证 PR）。
- **技术债**：销账——reasoning/cache 未闭环、能力虚标（Gemini seed/penalties、Ollama top_p）、Retry 429 漂移（机器断言固化）；结转——O-1（OpenAI reasoning 真实端点核验 → E2E-001）、O-2/O-3/O-4（Anthropic adaptive、thoughtSignature、Gemini thinkingLevel → 0.2 候选）、E-004（FinishReason 扩展性 → 0.2 评估）；卫生项——8 个已关账任务旧分支残留（已于 2026-08-02 清理）。
- **CI 战功与漏报**：七项门禁全程有效（热点基线审批、semver gate 守 0.1.2 基线、新 head 重跑、CAP-001 可证伪人机一致性断言）；无记录在案漏报。流程资产：设计依据回证前置、合并预检 + 标题第二关、机械代笔 + 逐字节核验治理通道。
- **规格修订**：无新增（SPEC-004 角色更换已登记）。
- **下一阶段裁定**：M2 退出判据满足、无开放 P0/P1——架构师裁定放行 M3（2026-08-02），PRX-001 转 READY 并下发。Owner 口径（2026-08-02）：阶段放行属架构师裁定职责，Owner 不否决即通过；仅方向/范围/发布时间/成本/风险实质变化时呈 Owner 并附架构师建议。

### M3 相位复盘（2026-08-03，架构师复盘完成；M4 放行待 Owner 批准）

- **计划/完成**：5/5（PRX-001..005 实现与 STATE 关账各 5 笔全部 DONE；M3 出口标准「五项任务 DONE，安全负例全部通过」达成）。
- **一次通过率**：实现 PR 一次通过 4/5。唯一返工 = PRX-005 MUST-FIX-1：实现选型误将 `DefaultBodyLimit` 置 `usize::MAX`（以读时内存保护换 413 错误形状——安全回归），评审拦截后经 `ae0ad74` 熔断修复（恢复读时硬上限 + map 中间件协议形状化）；附 MUST-FIX-2 台账行数两笔修正（3326→3327、1889→1884，hunk 数学独立验算闭合）。评审门禁有效性正面案例：回归未进 main。
- **越界**：范围/依赖/测试/安全红线零越界（`subtle` 为全相位唯一新依赖边、架构师预批；公开 DTO 零改动；PRX-001..004 wire 逐字节守恒由机器断言守住）。流程偏差三笔如实记档：① 合并令「gh 默认取 PR body」指令缺陷（架构师侧，merge commit body 缺失一笔，功能零损失）→ 防呆固化：此后合并令一律显式 `--body-file`，两笔合并 body 完整闭合验证生效；② 执行侧本地误提 main（即刻 reset 自纠、零损失、如实熔断）→ checklist 新增 `git branch --show-current`；③ 架构师 DOC-002 find-string 依赖误记 → 执行侧按文件实际字节应用并熔断报告（正确纪律，正面先例）。
- **技术债**：销账——audit P2-4 两协议错误形状不对称（PRX-004/005 修复）、代理默认 CORS/认证/health 暴露面（PRX-001/002）、截流伪造终止与 reasoning 静默丢弃（PRX-003/004）。结转——热点物质膨胀：proxy/mod.rs 3097→3327、anthropic_proxy.rs 1520→1884（测试与生产同文件为结构性根因，ARC-001/002 + 测试外迁待 M4）；`package_guard` 退出码盲区（A5，REL-002 MUST 候选）；reasoning 验证仍 fixture 级（E2E-001，M5）。
