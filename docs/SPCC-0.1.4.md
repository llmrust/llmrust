...
| 热点台账 | 3327 / 1884 / 1524 / 1483 / 1455 / 1132 / 1080，threshold 800 | `tests/hotspot_ledger.json` 逐项吻合 |
...
|---|---|---|---|---|---|---|---|
| `SPEC-010` | 治理（不计入任务数） | `PLANNED` | Owner 批准本规格 + 账号身份分离 | — | — | — | — |
...
- **待裁（开工前必须解决）**：本卡范围含"接入新厂商"，与 §1.6 非目标「**新增 Provider**」（`docs/SPCC-0.1.4.md:133`）及 `CAP-004` 禁止范围同句（`:520`）存在**字面冲突**。两种读法须由 Owner/架构师裁定：**(a)** §1.6 的"新增 Provider"指**新增协议族/新增 `src/providers/*.rs` 实现** → 本卡的"目录数据 + 既有协议"不违反，仅需在规格里写明；**(b)** 指**任何新厂商接入** → 则须修订 §1.6（Owner 令要求支持这些厂商）或把 R3 移出 0.1.4。**在裁定前，本卡不得开工。**
- **消化发现**：`#205` R2 + R3。
...
| `FND-R2` | REV-001 | P2 | `R-C` | 已开 #198（FIX-R2）→ 并入 `ERR-002` 核验 | 已开卡 |
