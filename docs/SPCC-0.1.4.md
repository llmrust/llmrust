...
| Rust 代码 | 19,513 行 / 41 文件 | **编制时快照**（本表锚定主干 `f4eccb4e` @ 2026-08-03，本地 `wc -l`）。**现值（2026-09-11 复测）**：`src` **25 文件 / 19,446 行**；同口径合计（`src`+`tests`+`examples`）**53 文件 / 27,315 行**。**原 41 文件的分口径未记录，执行侧无法据实复原，故不猜** |
| 测试函数 | 380 个 `#[test]` | **编制时快照**（同上）。**现值（2026-09-11 复测）**：`cargo test --all-features` → **19 组 / 563 passed / 0 failed**（原口径为 grep 计数 `#[test]`，与"通过数"不是同一口径，**不混报**） |
...
| `0.1.4 / N1 Capability Truth` | `#9` | 能力声明进入代码并被门禁保护 | **6/6** | 100% | `DONE` | —（N0 DONE） | 三处一致门禁上线且负例可红；**且 `CAP-007`（能力表机读化 + `CAPABILITIES.md` 由表生成）完成** |
...
| `0.1.4 / N5 Release` | `#13` | 审计并发布 0.1.4 | **3/3** | **100%** | `DONE` | —（**待 N0–N2 DONE**，随 `N3`/`N4` 移出而变更） | crates.io / docs.rs / GitHub tag 三方一致 |

> **决策记录：`0.1.4` 范围收缩（Owner 2026-09-11 决定，读法 B）**
>
> **决定**：`N3 Evidence Depth` 与 `N4 Governance Scale` **整块移出 0.1.4**，顺延至 **0.1.5**；
> `N5 Release` 的前置由「待 N0–N4」改为「**待 N0–N2 DONE**」。`N0`/`N1`/`N2` 已完成并作为本版范围。
>
> **理由（Owner 给定）**：**主项目 `yesagent` 有交期**；`N0`（门禁可信）+ `N1`（能力与钱线）+ `N2`（真实缺陷）
> 三者已足以让主项目**用上新能力且有门保护**，而 `N3`（他证）与 `N4`（治理伸缩）**不阻挡 API 可用性**。
>
> **代价（已明示，不粉饰）**：本版**不含 live-endpoint 证据**——即 `CAP-005`（缓存断点）只证明「**断点发得出**」，
> **未证明**真端点 `cached_tokens > 0`，故**省钱是否为正未被实测**。`RC-002` 与本版发布说明**必须如实写明**此点
> （对应规格 §1.4「真实端点证据到位前，任何『修好了』的结论仍然是自证」）。
> 相关卡 `E2E-002` / `FIX-001` / `GOV-001..003` 一并顺延，见 §11.1.3 各行标注。
>
> **回滚条件**：若 Owner 在打 tag 之前给出真端点凭据并完成缓存一条 E2E，则可在 `REL-004` 之前把该证据回填，
> 并按 `RC-002` 重开判据——**该窗口随 `REL-005` 打 tag 关闭**。
...
flowchart TD
    ...
    N1 --> N2["N2 静默失败"]
    N2 --> N5["N5 发布"]
    N0 -. "移出 0.1.4" .-> N3["N3 验证深度 → 0.1.5"]
    SPEC -. "移出 0.1.4" .-> N4["N4 治理可伸缩 → 0.1.5"]
```

> **范围收缩后的依赖图（2026-09-11，Owner 决定读法 B）**：`N5` 的入边只剩 **`N0`/`N1`/`N2`**——
> `N3`/`N4` 已移出本版（虚线标出），故**不再阻塞发版**。原图（`N3`/`N4` → `N5`）保留于 git 历史。

**为什么 N0 必须最先做**：后续所有里程碑的验收都依赖门禁给出的绿灯。若门禁本身是 fail-open 或单向的，N1–N4 的"全绿"不构成证据。**先修验收机制，再修被验收物** —— 这是本版递归改进的第一原则。
（**注**：此处"N1–N4"是**通则表述**（适用于全部里程碑），**不是本版的前置条件**；本版前置见 §11.1.2 `N5` 行 = **N0–N2 DONE**。）

**为什么 N3 与 N1 并行而非串行**（**本版已不适用，保留为设计依据**）：`E2E-002` 需要真实端点预算与 Owner 授权，前置周期长，不应被 N1 的代码工作阻塞。且 N3 的产出（真实端点行为证据）是 N1 `CAP-004` 中 `verified` 格子的输入 —— 两者交汇于 N5 前的对账。
**本版实际结果**：`N3` 因**主项目交期**被整块移出（见 §11.1.2 决策记录），故 `CAP-004` 的 `verified` 格子在本版**只有 `local-fixture` 证据**，`live-endpoint` 一列**留空并如实标注**。
...
| `CAP-002` | N1 | `DONE` | N0 DONE | —（未开卡，见 §11.1.4 披露） | #225 | `788a58d` | 本 PR |
| `CAP-003` | N1 | `DONE` | `CAP-002` | —（未开卡） | #226 | `6d2009c` | 本 PR |
| `CAP-004` | N1 | `DONE` | `CAP-003` | —（未开卡） | #227 | `7c3865c` | 本 PR |
...
| `ERR-001` | N2 | `DONE` | N1 DONE | —（未开卡） | #228 | `41de972` | 本 PR |
| `ERR-002` | N2 | `DONE` | `ERR-001` | —（未开卡） | #229 | `657e5b2` | 本 PR |
| `ERR-003` | N2 | `DONE` | N1 DONE | —（未开卡） | #230 | `09bd9c3` | 本 PR |
| `ERR-004` | N2 | `DONE` | `ERR-001` | —（未开卡） | #231 | `d905a55` | 本 PR |
| `ERR-005` | N2 | `DONE` | N1 DONE | —（未开卡） | #232 | `498c1e2` | 本 PR |
| `E2E-002` | N3 | `DEFERRED`（→ 0.1.5） | N0 DONE + Owner 预算授权 | — | — | — | 本 PR（随范围收缩） |
| `FIX-001` | N3 | `DEFERRED`（→ 0.1.5） | 平台侧 401 定性；**2026-08-31 硬时限** | — | — | — | 本 PR（随范围收缩） |
| `DOC-003` | N3 | `DEFERRED`（→ 0.1.5） | `CAP-004` | — | — | — | 本 PR（随范围收缩） |
...
| `CAP-002` | —（未开卡） | #225（`Capabilities` 载体 + `Provider::capabilities()`，带默认实现） | Security Gates `34593984608` / CI `34593984637`（均 SUCCESS，main 推送） | `788a58d` | 本 PR | N1 `6/6` | 待架构师核验 |
| `CAP-003` | —（未开卡） | #226（统一入口能力裁决：Pass/Warn/Reject，六处手工调用点迁走） | Security Gates `34596039227` / CI `34596039237`（均 SUCCESS，main 推送） | `6d2009c` | 本 PR | N1 `6/6` | 待架构师核验 |
| `CAP-004` | —（未开卡） | #227（能力矩阵三处一致门：运行时↔JSON↔文档） | Security Gates `34597208389` / CI `34597208452`（均 SUCCESS，main 推送） | `7c3865c` | 本 PR | N1 `6/6` | 待架构师核验 |
| `ERR-001` | —（未开卡） | #228（三处静默 `json!({})` 改留痕降级 + `http.rs` 回落写明丢失配置） | Security Gates `34598734051` / CI `34598733996`（均 SUCCESS，main 推送） | `41de972` | 本 PR | N2 `5/5` | 待架构师核验 |
| `ERR-002` | —（未开卡） | #229（`error_kind` 真实分类映射 + 截断移出 `match` 成结构性全覆盖） | Security Gates `34600061443` / CI `34600061309`（均 SUCCESS，main 推送） | `657e5b2` | 本 PR | N2 `5/5` | 待架构师核验 |
...
> `CAP-003`（统一入口裁决；**开发中真实踩坑**——首版把「未声明」与「声明为 unsupported」一视同仁，
> `proxy::tests` 三条既有测试立刻红，遂引入 `declared` 标记区分并补回归防线测试）、
> `CAP-004`（三处一致门；**写门之前先量真值表，抓到 deepseek/openrouter 的 `reasoning` 声明与 JSON/文档不一致**，已修正），
> 以及已在库的 `CAP-005/006/007`。
> ② **`N2` 5/5**：`ERR-001`（三处 `json!({})` 静默改写工具调用 → 留痕降级；测试断言**痕迹不含参数原文**）、
> `ERR-002`（`error_kind` 硬编码 → **9 类映射表**；≤200 截断**移出 `match`** 成为结构性全覆盖）、
> `ERR-003`（`UNSPECIFIED` 不再冒充「正常结束」，**变体集合实测未增**）、
> `ERR-004`（`Retry-After` 两种格式 + **60s 上限**；承载走**带默认实现的 trait 方法**——因 `LlmError` 非 `#[non_exhaustive]`，
> 加字段/加变体都会破坏下游）、`ERR-005`（认证 trim 一致；**与 `FIX-001` 的干扰关系已逐条排除**）。
> ③ **本版不含 live-endpoint 证据**（`N3` 已移出，见上方决策记录）——**不得**把上述任一卡表述为「真端点已验证」。
> ④ **`CAP-002/003/004` 与 `ERR-001..005` 均未开 Issue**，工作以实现 PR 直接落地（与 `GRD-001..004` 同例），
> 是否补开以对齐八列口径，**请架构师裁定**（本 PR 不自行补开）。
...
**背景**：0.1.3 期内两次熔断表层原因不同，但背景相同 —— `docs/SPCC-0.1.3.md` 单文件 **1427 行 / 149.8 KB**
（**2026-09-11 复测**，`wc -l` 口径依 `E-101`；`E-101` 记录 0.1.3 编制期为 1421 行，其后 `SPEC-010` 步骤②
「0.1.3 规格加只读头」+6 行，故现值高于当时快照），超出单次工具读取窗口（实测读到 63% 被截断），
导致凭压缩记忆写指令、前提为假。M3、M4 复盘已两次建议小文件化，两次未执行。**本版不再推迟。**
...
- **初始状态/依赖/分支**：`PLANNED`；**N0–N2 DONE**（原为 N0–N4 DONE，随 `N3`/`N4` 移出而变更）；`task/RC-002-release-audit`。
...
**计数核对（2026-09-11 复测；**本节原表述正确，先前一版"更正"是执行侧数错，已撤回**）**：

| 口径 | 数 |
|---|---|
| **表内数据行（实测）** | **25** |
| 其中 来源=`REV-001` | 12 |
| 其中 来源=`REV-001 交集`（`F-C1` ≡ `FND-R1`） | 1 |
| 其中 来源=`架构师复审` | 7 |
| 其中 来源=`0.1.3 结转` | 5 |
...
```powershell
# 取 "## 14." 起至 "计数核对" 前的所有以 "|" 开头、非分隔行、非表头的行 —— 共 25 行
$rows = $f[($s)..($e-2)] | ? { $_ -match '^\|' -and $_ -notmatch '^\|---' -and $_ -notmatch '^\| 发现 ' }
```

> ⚠️ **撤回记录（`E-103` 自查）**：本单**较早一版**曾在此处写"表内 23 行"并据此提出
> 「(a) 计数写错 / (b) 表缺 3 行」的裁定请求。**那是我数错**：我用 `^\| \`` 过滤，
> 而表中 `Gemini cache token 缺口` 与 `发布可见性轮询 300s` **两行的首列是纯文本、无行内代码**，
> 被该过滤漏掉 ⇒ 23 是**我的口径错**，不是表的错。
> **表内实为 25 行，与本段原有表述「21 + 4」一致** ⇒ **既非 (a) 也非 (b)，§14 表完整、无覆盖缺口。**
> **架构师无需就此事裁定**；若已据旧文启动裁定，请以本段为准。
> **教训**：计数口径必须写进查询式并**自证能覆盖边界行**——本段现已附上可复跑命令。
