...
> **目标文件（2026-08-03 编制时快照）**：`src/proxy/mod.rs`（3327 行）、`src/proxy/anthropic_proxy.rs`（1884 行）
>
> ⚠️ **行数已漂移（2026-09-11 复测，`wc -l` 口径）**：`mod.rs` = **3558**（+231）、
> `anthropic_proxy.rs` = **1886**（+2）。**下文全部行区间均按 2026-08-03 快照编号，
> 与当前文件不再对齐**——`0.1.4` 期间 `N0`–`N2` 在 `mod.rs` 上有净增（§1.6 明文排除拆分，故未拆）。
> **拆分开工（`0.2.0`）前必须重测并重编号**，不得直接照本稿行号下手。
