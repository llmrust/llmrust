...
/// The `content_block` carried by a `content_block_start` event. A `tool_use`
/// block opens a streamed tool call and carries its `id` and `name`; text
...
/// Accumulates streamed tool-call fragments from an Anthropic stream. Claude
/// opens each tool call with a `content_block_start` (`tool_use`) carrying the
/// call `id` and `name`, streams its JSON arguments via `input_json_delta`
/// fragments on `content_block_delta`, and signals completion on
/// `message_delta`. Blocks are keyed by their stream `index`.
#[derive(Default)]
...
/// [`StreamChunk`]s, threading an [`AnthropicToolAccumulator`] so streamed tool
/// calls can be reconstructed across events. Lines are guaranteed complete by
/// [`line_stream`].
///
