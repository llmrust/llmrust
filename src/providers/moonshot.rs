...
// --- OpenAI-compatible types for Moonshot ---

#[derive(Serialize)]
struct MoonshotRequest<'a> {
    model: &'a str,
    messages: &'a [MoonshotMessage],
...
impl From<&Message> for MoonshotMessage {
...
impl Provider for MoonshotProvider {
    async fn chat(&self, req: &ChatRequest) -> Result<ChatResponse> {
        let messages: Vec<MoonshotMessage> =
            req.messages.iter().map(MoonshotMessage::from).collect();

        let body = MoonshotRequest {
            model: &req.model,
            messages: &messages,
        ...
        if !status.is_success() {
            ...
            let msg = serde_json::from_str::<MoonshotErrorBody>(&text)
        ...
        let parsed: MoonshotResponse = resp
            .json()
            .await
        ...
        let stream = resp
            ...
            .map(|chunk_result| {
                ...
                for line in text.lines() {
                    ...
                    if let Ok(parsed) = serde_json::from_str::<MoonshotStreamChunk>(data) {
