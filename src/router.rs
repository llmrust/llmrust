...
impl Router {
    ...
    fn resolve(&self, group: &str) -> Vec<String> {
        ...
        match self.strategy {
            ...
            RoutingStrategy::RoundRobin => {
                ...
                base[start..].iter().chain(&base[..start]).cloned().collect()
            }
        }
    }

    /// Send a single-message chat request to `group`, falling back through its
    /// deployments on transient failures.
    pub async fn chat(&self, group: &str, prompt: &str) -> Result<ChatResponse> {
        self.chat_with(group, ChatRequest::new("", prompt)).await
    }

    /// Like [`Router::chat`] but with a full [`ChatRequest`]. The request's
    /// `model` field is set per-deployment, so any value on `req.model` is
    /// ignored.
    pub async fn chat_with(&self, group: &str, req: ChatRequest) -> Result<ChatResponse> {
        let deployments = self.resolve(group);
        let mut last_error: Option<LlmError> = None;

        for model in &deployments {
            match self.client.chat_with(model, req.clone()).await {
                Ok(resp) => return Ok(resp),
                Err(e) if should_failover(&e) => last_error = Some(e),
                Err(e) => return Err(e),
            }
        }
    ...
    /// Open a streaming chat request to `group`, falling back through its
    /// deployments on transient connection failures.
    pub async fn stream(
        &self,
        group: &str,
        prompt: &str,
    ...
    /// Like [`Router::stream`] but with a full [`ChatRequest`].
    pub async fn stream_with(
        ...
        for model in &deployments {
            match self.client.stream_with(model, req.clone()).await {
...
mod tests {
    ...
    async fn falls_back_on_transient_error() {
        let client = Arc::new(LmrsClient::new());
        client.set_custom("bad", Arc::new(FailingProvider::new(503))).await;
        client.set_custom("good", Arc::new(OkProvider::new("good"))).await;
    ...
    async fn permanent_error_is_not_retried() {
        let client = Arc::new(LmrsClient::new());
        client.set_custom("bad", Arc::new(FailingProvider::new(400))).await;
        client.set_custom("good", Arc::new(OkProvider::new("good"))).await;
    ...
    async fn all_deployments_fail_returns_last_error() {
        let client = Arc::new(LmrsClient::new());
        client.set_custom("a", Arc::new(FailingProvider::new(500))).await;
        client.set_custom("b", Arc::new(FailingProvider::new(503))).await;

        let router = Router::new(client).route("grp", ["a/m", "b/m"]);
        let err = router.chat("grp", "hi").await.unwrap_err();
    ...
    async fn round_robin_rotates_start() {
        let client = Arc::new(LmrsClient::new());
        client.set_custom("a", Arc::new(OkProvider::new("a"))).await;
        client.set_custom("b", Arc::new(OkProvider::new("b"))).await;
    ...
    async fn stream_falls_back_on_transient_error() {
        let client = Arc::new(LmrsClient::new());
        client.set_custom("bad", Arc::new(FailingProvider::new(502))).await;
        client.set_custom("good", Arc::new(OkProvider::new("stream-ok"))).await;
